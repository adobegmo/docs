 ...
 * /auth/session means signed-in-but-not-on-the-allowlist. This is the trimmed,
 * SigV4-free descendant of spectrum-hub scripts/utils/ims.js.
 */
...
// The client-side script, as a template. Only two values are interpolated - the
// public OAuth client id and the imslib environment - and both come from trusted
// server config, never from the request, so there is no user input to escape.
const clientScript = (clientId, environment) => `
  ...
  // imslib reads window.adobeid on load, so it must be appended only after the
  // config above is in place - hence a dynamic script rather than a static tag.
